...
получателей.(Готов АПИ для обоих запросов)
...
(реализовано через две раздельные очереди, приоритет определяется 
при отправке запроса на массовую рассылку в поле priority transactional|marketing)
* Детализация статусов доставки: Система должна предоставлять API для
запроса истории (?что значит для запроса истории.?) 
и текущего статуса всех уведомлений конкретного
подписчика. Статусы:
  ...
  (изменено на error для всех типов сбоев при отправке сообщения в шлюз)

🛡️ Нефункциональные требования (Архитектура и качество)
* Гарантия доставки сообщений (Reliability):
    ...
    будет реализация exactly-once на уровне бизнес-логики. (непонятна формулировка, уточнить)
...
же сообщения при дублировании запросов от вызывающего сервиса. (нужно уточнить логику дедубликации)
...
* Cloud Native и развертывание:
    + 🟡 Упаковка сервиса в Docker-образ. (добавить автозапуск миграций и опрос статуса доставки)
